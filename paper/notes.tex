...
Starting from the branching equation again (but dropping the competition term,
$\chi_i$):
...
The spatial part of the system can be halted if $D\rightarrow
0, \gamma_{i,j}\rightarrow 0$ as
$f(\gamma_{i,1},\gamma_{i,2},...,\gamma{i,M})\rightarrow 1$. That is, if, for
a particular axon, $i$, a sampling of $\rho_j$ is close to $f(\gamma_{i,j})$
for each $j$, then $D$ is reduced to zero, and likewise, the second term also
reduces to zero. $f()$ is probably $1-$ a (possibly noisy) normalized Gaussian
function, or a capped Gaussian (with height capped by a sigmoid?), with mean
$(\gamma_{i,1},\gamma_{i,2})$ and a width based on the differences between
$\gamma_{i\pm 1,j}$. Something like:
...
$f$ could be thought of as a functional:
$f_i[\gamma_{i,1},\gamma_{i,2}](\mb{x})$ (if that's the right way to write
it...).

Let's try to write f out. See gaussians.m initially.
