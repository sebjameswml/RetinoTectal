...
The border effect ($\mathbf{B}$ in Eq.\,\ref{e:M}) was based on gradient-following by assuming that there is some other molecular signal which acts on all branches near the boundary of the tectal tissue. For a branch with position $(x,y)$, $\mathbf{B}$ is given by:
%
...
\caption{(Supplementary) Response of the model to noise. \textbf{A} Adding 3\% gradient measurement noise (normally distributed, standard deviation, $\sigma = 0.19$) has negligible effect on the pattern. \textbf{B} 3\% retinal receptor noise ($\sigma = 0.08$). \textbf{C} 3\% tectal ligand noise ($\sigma = 0.08$). \textbf{D} Combined effect of retinal receptor, tectal ligand and gradient measurement noise is dominated by the tectal ligand noise.}
\label{f:noise}
\end{figure}
