...
axonal agent.
%
Here, we investigated whether a purely self-organising model based on receptor
and ligand interactions could be found, that would successfully reproduce both
the normal development of the projection and its behaviour under the many
...
interactions) combined with receptor-receptor mediated axonal repulsion can
account for a broad range of experimental results, with information only being
transferred by nearest-neighbour interactions.
}
...
retina primarily excite cells adjacent in the tectum. This indicates that
during development there must exist a mechanism which ensures the correct
arrangement of the axons which leave the retina and connect to cells in the
tectum.
