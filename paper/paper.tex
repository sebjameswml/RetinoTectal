...
as \emph{branches}) which carry a set of 4 receptors, $r_i$, and 4 ligands, $l_i$,
...
tectal ligand expression, $L$. Although several studies model tectal ligand
...
X). Correspondingly, we leave open the possibility that tectal ligand
...
growth cone (forward signalling) can lead to one of two effects; the
...
binding model, and set the chemotactic movement vector of the growth cone $b$ at
...
\end{equation}
%
where $r_{i,b}$ is the receptor expression on axon branch $b$ for
ligand-receptor pair $i$, $\nabla L_i$ is the gradient of expression of ligand
$i$ on the tectum and $F_i$ denotes the direction of the interaction induced
when a molecule of ligand $i$ binds to a receptor $i$ molecule. $F_i$ takes
the value -1 for a repulsive interaction or 1 for an attractive interaction.
...
interaction, $\mathbf{I}_b$, acting on axon branch $b$, is given by
%
\begin{equation}
\mathbf{I}_b = \frac{1}{|B_b|} \sum_k \hat{\mathbf{x}}_{kb}\,Q(d_{kb})
\end{equation}
%
