...
successfully reproduce the wildtype and manipulation results. To investigate,
we studied each competitive mechanism alongside chemoaffinity in turn, setting
$m_{\!_I}=m_{\!_J}=0$ to give a `$\mathbf{GC}$' model, $m_c=m_{\!_J}=0$ to
give a `$\mathbf{GI}$' model or $m_c=m_{\!_I}=0$ to give a `$\mathbf{GJ}$'
model.
...
(Fig.\,\ref{f:ch}E) at simulation time 1000. The parameters returned were $m_c$
= 0.09959, $m_{\!_G}$ = 0.003841 and $r_c$ =
0.39918. Fig.\,\ref{f:nonexistent_yet} shows a simulation with these parameters.
