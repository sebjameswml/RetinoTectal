...
interaction between EphA receptors, which is defined (with receptor type $i$
standing for EphA) as:
...
suggest that each receptor ligand pair involved in the retinotectal system
could mediate a repulsive interaction. DO WE NEED TO? Taken alone, the EphA
interaction between one branch, $b$, and another, $k$, will cause a repulsion
if $k$ originated more \textbf{nasally?} than $b$. What about a branch
originating on the temporal side of $b$? This could be handled by the
interaction between $k$ and $b$ and may not require an additional interaction.
