...
\title {
  ...
  \emph{or} \\
  Can the retinotectal projection be modelled exclusively by receptor-ligand interactions?
}
\date{} % No date please
\author{\Authors}
...
receptor-ligand interaction, $\mathbf{I}_b$ and a `border effect',
$\mathbf{B}_b$, which acts to retain branches within the tectal region:
%
\begin{equation}
\mathbf{M}_{b} = m_1 \mathbf{G}_b + m_2 \mathbf{C}_b + m_3 \mathbf{I}_b + m_4 \mathbf{B}_b
\end{equation}
%
evaluated at time $t$. $m_1$--$m_4$ are scalar parameters.
