...
First, we describe the behaviour of a model which has only the gradient-based
chemoaffinity effect and no axon-axon interaction or border effect
(i.e.~$m_2=m_3=0$). This model is run with $n=8$ branches per RGC axon. All
receptor-ligand signals induce repulsive interactions (i.e.~gradient
descent). Receptor and ligand expressions are as shown in
Fig.\,\ref{f:ex}. Fig.\,\ref{f:ch}A gives a colour map of retinal ganglion
cell soma locations, with the colour red encoding $x$ position on the
temporal-nasal axis (with red signifying nasal positions). Green encodes $y$
position on the dorso-ventral axis.
%
Fig.\,\ref{f:ch}A+ shows the expected final location of each RGC axon,
according to experimental observations in a `fishnet' plot, which draws lines
between adjacent axons.
%
Fig.\,\ref{f:ch}B shows the position of
...
\begin{figure}
\includegraphics[width=\linewidth]{./images/fig_chemo.png}
\caption{Chemoaffinity model}
\label{f:ch}
\end{figure}

Fig.\,\ref{f:ch}C shows the RGC axon centroids; the mean location of $n$
branches per axon. Because all $n$ branches for each RGC have identical
receptor expression and they experience only the chemoaffinity effect, each
branch attains almost the same final position and so the centroid graph
strongly resembles the graph of all the branches. Fig.\,\ref{f:ch}C also shows

\subsubsection*{Wildtype}
\subsubsection*{Tectal ablation etc}

\subsection*{Chemoaffinitiy with axon-axon interaction}
