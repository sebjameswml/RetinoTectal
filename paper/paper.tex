...
full tectum. Fig.\,\ref{f:ch}F shows two pattern metrics. The first metric,
$\epsilon$, is the sum of squared distance errors of the axon centroids, with
$\mathbf{x}_{k}$, the location of the centroid of the branches of axon $k$ and
$\mathbf{x}'_{k}$ the target tectal location for axon $k$:
%
...
the 180\degree~example in row B, in whcih the caudal half of the rotated patch
...
(`$\mathbf{J}$'). \citet{simpson_simple_2011} found that both a space based
competition and a receptor-receptor signalling mechanism were required to
reproduce the full range of surgical and genetic manipulations explored in
that work. The first of these, space-based competition, offers no mechanism by
which axon growth cones are able to communicate and the second mechanism
explored by \citet{simpson_simple_2011} was implemented only for a single axis
in the model (EphA receptors, expressed in a gradient along the XX retinal
axis). We wondered whether a single signalling mechanism could provide a
suitable competitive, repulsive effect. To investigate, we compared both of
the \citet{simpson_simple_2011} effects along with a third possible signalling
mechanism involving the same receptor-ligand interactions employed by the
gradient following mechansim, because the ligands expressed on the tectum are
also expressed by the retinal ganglion cells.
...
\mathbf{C}_b = \frac{1}{|B_{b,c}|} \sum_k \hat{\mathbf{x}}_{kb} W
\end{equation}
where the distance-dependent weight $W_c$ is given by
\begin{equation}
W_c(d_{kb}) = \begin{cases}
      1 - \frac{d_{kb}}{2r_c}   & d_{kb} \leq 2r_c \\
     0 & d_{kb} > 2r_c
