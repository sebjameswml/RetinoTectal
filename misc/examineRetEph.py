...
def examineRetEph (knockin_ephax = 0, knockdown_epha4 = 0):
    ...
    ax1.plot (x, ephrinA, linestyle=':', color=C.black, label='ephrinA')
    #ax1.plot (x, ephrinA_free, linestyle='-', color=C.black, label='ephrinA free for EphAx')
    #ax1.plot (x, ephrinA_binds_EphAx, linestyle='--', color=C.royalblue, label='ephrinA bound to EphAx')
    #ax1.plot (x, _p_epha4, linestyle='--', color=C.red, label='ephrinA bound to EphA4')
    ax1.legend()
    ax1.set_xlabel('N {0} retina {0} T'.format(u"\u27f6"))
    ax1.set_ylabel('Expression/Interaction')
    ax1.set_ylim(0,5)
    ax1.text(0.03, 0.7, 'EphAx knockin {0}'.format(knockin_ephax))
    ax1.text(0.03, 0.4, 'EphA4 knockdown {0}'.format(knockdown_epha4))

    #ax2.plot (x, ephrinA, linestyle=':', color=C.black, label='ephrinA')
    ax2.plot (x, _epha4, linestyle=':', color=C.red, label='EphA4')
    #ax2.plot (x, EphAx, linestyle='-', color=C.royalblue, label='EphAx')
    ax2.plot (x, EphA4_free, linestyle='-', color=C.red, label='EphA4 free')
    ax2.plot (x, _p_epha4, linestyle='--', color=C.red, label='EphA4 cis bound')
    ax2.legend()
    ax2.set_xlabel('N {0} retina {0} T'.format(u"\u27f6"))
    ax2.set_ylabel('Expression/Interaction')
    ax2.set_ylim(0,5)

    ax3.plot (x, 1/EphA4_free, linestyle='-.', color=C.violetred, label='Cluster size (1/EphA4 free)')
    ax3.plot (x, EphAx/EphA4_free, linestyle='-', color=C.purple2, label='Signal strength (EphAx/EphA4 free)')
    ax3.legend()
