#
# This version does EphA4 knockdown on the EphA4 constant value.
#
...
def examineRetEph():
    ...
    ax2.plot (x, EphA4_free_kd, linestyle='-', color=C.springgreen3, label='EphA4 free (kd)')
    ax2.legend()
    ...
    ax3.plot (x, 1/(EphA4_free_kd), linestyle='-.', color=C.springgreen3, label='Cluster size (kd)')
    ax3.plot (x, EphAx/EphA4_free, linestyle='-', color=C.royalblue, label='Signal (WT)')
    ax3.plot (x, EphAx/(EphA4_free_kd), linestyle='-', color=C.springgreen3, label='Signal (kd)')
    ...
    ax3.plot (x, (EphAx+ki)/(EphA4_free_kd), linestyle='-', color=C.goldenrod1, label='Signal (ki+kd)')
    ax3.legend()
